...
def get_optimizer(training_args, other_args, model):
    ...
    diff_part = ["exp_att", "Wr0", "Wr1", "hist_lstm", "hist_att"]

    if other_args.diff_lr is True:
        logger.info("------------ DIFF learning rate on --------------------------- ")
        optimizer_grouped_parameters = [
            {
                # 有weight decay 其他part
        ...
        logger.info(" ........... BART learning rate on .................... ")
        no_decay = ["bias", "LayerNorm.weight"]
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if
...
def train(train_dataloader, eval_dataloader, test_dataloader, model, training_args, other_args, label_vocab, tokenizer):
    ...
    for epoch in trange(int(training_args.num_train_epochs), desc="Epoch"):
        ...
        for data in tqdm(train_dataloader, desc="Iteration", smoothing=0.05):
            model.train()
            outputs = model(**data)
            loss, gen_loss = outputs.loss, outputs.gen_loss
...
def evaluate(training_args, other_args, eval_loader, model, eval_or_test, label_vocab, tokenizer):
    ...
    for batch in tqdm(eval_loader, desc=eval_or_test):
        ...
        with torch.no_grad():
            inputs = {'input_ids': batch[0], 'attention_mask': batch[1], 'speakers': batch[3],
                      'speaker_feels_mask': batch[4], 'pre_encoded_features': batch[5],
                      'dia_hist_feat_seqs': batch[6], 'is_self_seq': batch[7],
                      'pre_encoded_demon_exp': batch[8],  'auxi_tgt_sentences': batch[9]}
    ...
    # print(len(all_preds), all_preds[0])
    # print(all_preds)
    ...
    result = compute_acc_for_categories(all_preds, all_labels)
    ...
    if eval_or_test == 'predict':
        cases = []
        i = 0
        for pred, label in zip(all_preds, all_labels):
            # print(all_text_ids[i])
            cases.append({
                'text': tokenizer.convert_ids_to_tokens(np.array(all_text_ids[i])),
                'label': label_vocab['itos'][label] if label != -1 else 'none',
                'pred': label_vocab['itos'][pred]
            })
            # print(cases[-1]['text'])
            i += 1
        with open('badcase/%s.json' % (other_args.task_name), 'w', encoding='utf-8') as f:
            json.dump(cases, f)
